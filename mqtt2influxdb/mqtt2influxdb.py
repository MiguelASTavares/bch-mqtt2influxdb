...
class Mqtt2InfluxDB:
    def __init__(self, config):
        ...
        self._influxdb = influxdb_client.InfluxDBClient(
    ...
    def run(self):
        logging.info(
            "MQTT broker host: %s, port: %d, use tls: %s",
    ...
    def _on_mqtt_connect(self, client, userdata, flags, rc):
        ...
        if rc != paho.mqtt.client.CONNACK_ACCEPTED:
            logging.error(
                "Connection refused from reason: %s", lut.get(rc, "unknown code")
            )
            ...
            if topic_matches_sub(point["topic"], message.topic):
                if not msg:
                    ...
                    try:
                        ...
                        logging.error(
                            "parse json: %s topic: %s payload: %s",
                            e,
                            message.topic,
                            message.payload,
                        )
                ...
                ### Adding timestamp from the mqtt msg ###
                timestamp = payload.get("timestamp")
                if timestamp:
                    if isinstance(timestamp, int):
                        record["time"] = datetime.fromtimestamp(timestamp)
                    else:
                        try:
                            ...
                            logging.warning("Invalid timestamp format: %s", timestamp)
                ### ---------------------------------- ###
                ...
                if "fields" in point:
                    if isinstance(point["fields"], jsonpath_ng.JSONPath):
                        record["fields"] = self._get_value_from_str_or_JSONPath(
                            point["fields"], msg
                        )
                        ...
                        for key in point["fields"]:
                            if isinstance(point["fields"][key], dict):
                                ...
                                convFunc = getattr(
                                    builtins, point["fields"][key]["type"], None
                                )
                                if val is None:
                                    continue
                                if convFunc:
                                ...
                                if key == "value":
                                    if isinstance(val, bool):
                                        if (
                                            "type" in point["fields"]
                                            and point["fields"]["type"] == "booltoint"
                                        ):
                                            val = int(val)
                                elif key == "type":
                                    if val == "booltoint":
                            ...
                            if val is None:
                                logging.warning(
                                    "Unable to get value for %s" % point["fields"][key]
                                )
                                continue
                ...
                if "tags" in point:
                    for key in point["tags"]:
                        ...
                        if val is None:
                            logging.warning(
                                "Unable to get value for tag %s" % point["tags"][key]
                            )
                            continue
                        record["tags"][key] = val
                ...
                ### adding the time to the influx point ###
                try:
                    point = point.time(record["time"])
                    logging.warning("sucss")
                except:
                ...
                self._influxdb_write_api.write(
                    bucket=self._influxdb_bucket, record=point
                )
                ...
                if "http" in self._config:
                    ...
                    if action:
                        r = action(
                            url=self._config["http"]["destination"],
                            data=http_record,
                            auth=HTTPBasicAuth(
        ...
        if isinstance(param, str):
            ...
            for var in param.variables():
                ...
                if var.startswith("JSON__"):
                    ...
                    logging.error(
                        "unknown variable %s in parser expression %s!"
                        % (var, param.toString())
                    )
            logging.debug(
                "evaluating expression %s using the variables %s"
                % (param.toString(), str(vars))
            )
